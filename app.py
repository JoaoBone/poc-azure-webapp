...
@app.route("/check-blob")
def check_blob():
    try:
        containers = list(blob_service_client.list_containers())
        container_names = [c['name'] for c in containers]
        return f"✅ Connected to storage. Found containers: {container_names}"
    except Exception as e:
        return f"❌ Failed to connect to storage: {str(e)}"

# 🔍 Check if blobs exist under a client path
@app.route("/check-client/<client_name>")
def check_client(client_name):
    try:
        blobs = list_client_blobs(client_name)
        if not blobs:
            return f"ℹ️ No blobs found for client '{client_name}'."
        return f"✅ Found blobs for client '{client_name}': {blobs}"
    except Exception as e:
        return f"❌ Failed to list blobs for {client_name}: {str(e)}"

# Render full JSON contents for a client
